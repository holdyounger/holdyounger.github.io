...
BOOL InjectCode(HANDLE hProcess, int nSize)   //关键注入部分
{
...
int main(int argc, char *argv[])
{
	...
	wstring processName = L"OneForYou.exe"; // 进程名
	...
	int i = 0;
	memset(g_InjectionCode, 0, shellcode_byte.size());
	for (; i < shellcode_byte.size(); i++) {
		g_InjectionCode[i] = shellcode_byte[i];
	}
	g_InjectionCode[i+1] = '0x00';
	...
	printf("hprocess:%d\n", hProcess);
	InjectCode(hProcess, shellcode_byte.size());
